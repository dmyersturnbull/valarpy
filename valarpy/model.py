...
class CompoundNames(BaseModel):
    ...
    class Meta:
        ...
        indexes = (
            (('compound', 'external_source', 'name'), True),
        )
...
class OrderedCompounds(BaseModel):
    ...
    external_source = ForeignKeyField(db_column='external_source_id', null=True, rel_model=DataSources, to_field='id')
...
class PlateRuns(BaseModel):
    ...
    datetime_run = DateTimeField(index=True, null=True)
    days_post_fertilization = IntegerField(null=True)
    ...
    legacy_name = CharField(null=True, unique=True)
    ...
    sauronx_toml = IntegerField(db_column='sauronx_toml_id', null=True)
    suspicious = IntegerField()
...
class WellFeatures(BaseModel):
    ...
    lorien_config = IntegerField(db_column='lorien_config_id', index=True)
