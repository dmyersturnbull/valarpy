...
class SauronxRunParameters(BaseModel):
    name = CharField()
    sauronx_run = ForeignKeyField(db_column='sauronx_run_id', rel_model=SauronxRuns, to_field='id')
    value = CharField()

    class Meta:
        db_table = 'sauronx_run_parameters'
        indexes = (
            (('sauronx_run', 'name'), True),
        )
...
class WellTemplates(BaseModel):
    ...
    class Meta:
        ...
        indexes = (
            (('plate_template', 'well_range_expression'), True),
        )
